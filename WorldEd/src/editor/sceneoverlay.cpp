...
void LightSwitchOverlays::update()
{
    ...
    foreach (MapBuildingsNS::Building *building, mMapBuildings->buildings()) {
        if (!building->region().boundingRect().intersects(
                    QRect(0, 0, cellSize, cellSize)))
            continue;
...
{
    const int cellSize = mCellScene->world()->cellSize();
    QRectF r = mCellScene->renderer()->boundingRect(
                QRect(0, 0, cellSize, cellSize), 0);
    return r;
}

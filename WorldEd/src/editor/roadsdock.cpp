...
int RoadTypeModel::tileAt(const QModelIndex &index) const
{
    if (index.isValid()) {
...
public:
               ...
               const QModelIndex &index) const
    {
        ...
        const int roadTileIndex = static_cast<const RoadTypeModel *>(index.model())
                ->tileAt(index);
        if (roadTileIndex >= 0) {
            const QString tileName = RoadTemplates::instance()->roadTiles()
                    .value(roadTileIndex);
            const int separator = tileName.lastIndexOf(QLatin1Char('_'));
            const QString idText = separator >= 0
                    ? tileName.mid(separator)
                    : QString::number(roadTileIndex);
            const QFontMetrics metrics = painter->fontMetrics();
            const int textWidth = qMin(option.rect.width(),
                                       metrics.horizontalAdvance(idText) + 4);

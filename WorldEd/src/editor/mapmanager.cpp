...
void MapManager::mapLoadedByThread(Map *map, MapInfo *mapInfo)
{
    ...
    // TMX files embed their tileset declarations.  A map generated by
    // BMP-to-TMX therefore contains one temporary Tileset object for every
    // catalog entry.  When only 2x artwork exists, the portable TMX still
    // points at the conventional 1x path, so those temporary objects are
    // marked missing by MapReader even though the shared catalog has already
    // loaded the matching 2x images.
    //
    ...
    for (Tileset *embedded : embeddedTilesets) {
        Tileset *catalog =
                TileMetaInfoMgr::instance()->tileset(embedded->name());
        if (!catalog || catalog == embedded)
            continue;

        const bool compatible =
                catalog->tileWidth() == embedded->tileWidth()
        ...
        map->replaceTileset(embedded, catalog);
        delete embedded;
        ++relinkedTilesets;
    }

...
bool TexturePacker::pack(const TexturePackSettings &settings)
{
    ...
    for (const TexturePackSettings::Directory &tpd : settings.mInputImageDirectories) {
        ...
        const QSize packedTileSize = settings.mScale50 ? tileSize / 2 : tileSize;
        if (tpd.mImagesAreTilesheets
                && (packedTileSize.width() + settings.padding > settings.mOutputImageSize.width()
                    || packedTileSize.height() + settings.padding > settings.mOutputImageSize.height())) {
            mError = tr("Custom tile size %1x%2 does not fit in a %3x%4 output page.\n"
                        "Choose a larger texture size or reduce the custom tile size.")
                    .arg(packedTileSize.width())
                    .arg(packedTileSize.height())
                    .arg(settings.mOutputImageSize.width())
                    .arg(settings.mOutputImageSize.height());
            return false;
        }

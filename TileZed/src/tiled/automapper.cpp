...
QStringList AutoMapper::inputLayerNames() const
{
    QSet<QString> names = mInputRules.names;
    names.unite(mObjectInputRules.names);
    QStringList result = names.values();
    result.sort(Qt::CaseInsensitive);
    return result;
}

QStringList AutoMapper::outputLayerNames() const
{
    QSet<QString> names = mTouchedTileLayers;
    names.unite(mTouchedObjectGroups);
    QStringList result = names.values();
    result.sort(Qt::CaseInsensitive);
    return result;
}
...
bool AutoMapper::ruleLayerNameUsed(QString ruleLayerName) const
{
    return mInputRules.names.contains(ruleLayerName)
            || mObjectInputRules.names.contains(ruleLayerName);
}
...
bool AutoMapper::setupRuleMapTileLayers()
{
    ...
    foreach (Layer *layer, mMapRules->layers()) {
        ...
        if (layerName.startsWith(QLatin1String("input"), Qt::CaseInsensitive)) {
            ...
            if (layer->isTileLayer()) {
                ...
                mObjectInputRules.names.insert(name);

                if (!mObjectInputRules.indexes.contains(index)) {
                    mObjectInputRules.indexes.insert(index);
                    mObjectInputRules.insert(index, ObjectInputIndex());
                }

                if (!mObjectInputRules[index].names.contains(name)) {
                    mObjectInputRules[index].names.insert(name);
                    mObjectInputRules[index].insert(name, ObjectInputIndexName());
                }

                if (isNotList)
                    mObjectInputRules[index][name].listNo.append(layer->asObjectGroup());
                else
                    mObjectInputRules[index][name].listYes.append(layer->asObjectGroup());
            } else {
                error += tr("'input' and 'inputnot' layers must be tile or object layers!")
                        + QLatin1Char('\n');
            }
    ...
    if (mInputRules.isEmpty() && mObjectInputRules.isEmpty())
...
{
    foreach (const QString &name, mObjectInputRules.names) {
    ...
    for (int i = 0; i < mLayerList.size(); ++i) {
        ...
        foreach (Layer *layerKey, translationTable->keys()) {
            ...
            if (index >= mMapWork->layerCount() || index == -1 ||
                    ...
                    layerKey->type() != mMapWork->layerAt(index)->type()) {

                int newIndex = mMapWork->indexOfLayer(name, layerKey->type());
                if (newIndex == -1) {
                    const QString expectedType = layerKey->isTileLayer()
                            ? tr("tile layer") : tr("object layer");
                    mError += tr("%1\nOutput layer '%2' requires a %3 named "
                                 "'%4', but no compatible destination layer "
                                 "could be created or found.\n")
                            .arg(mRulePath, layerKey->name(),
                                 expectedType, name);
                    return false;
                }
...
const QRegion AutoMapper::getSetLayersRegion()
{
    ...
    foreach (const QString &name, mObjectInputRules.names) {
        const int index = mMapWork->indexOfLayer(name, Layer::ObjectGroupType);
        if (index == -1)
            continue;
        result |= tileRegionOfObjectGroup(mMapWork->layerAt(index)->asObjectGroup());
    }
    return result;
}
...
static bool compareObjectGroupTo(const ObjectGroup *setLayer,
                                 const QVector<ObjectGroup*> &listYes,
                                 const QVector<ObjectGroup*> &listNo,
...
QRect AutoMapper::applyRule(const int ruleIndex, const QRect &where)
{
    ...
    for (int x = minX; x <= maxX; ++x) {
        bool anymatch = false;
        QSet<QString> inputIndexes = mInputRules.indexes;
        inputIndexes.unite(mObjectInputRules.indexes);
        foreach (const QString &inputIndex, inputIndexes) {
            const InputIndex &ii = mInputRules[inputIndex];
            ...
            const ObjectInputIndex &oii = mObjectInputRules[inputIndex];
            foreach (const QString &name, oii.names) {
                const int index = mMapWork->indexOfLayer(name,
                ...
                const ObjectGroup *setLayer = mMapWork->layerAt(index)->asObjectGroup();
                allLayerNamesMatch &= compareObjectGroupTo(setLayer,
                                                            oii[name].listYes,
                                                            oii[name].listNo,
                                                            ruleInput, QPoint(x, y));
            }
...
static bool sameRuleObject(const MapObject *candidate, const MapObject *rule,
                           const QPoint &offset)
{
    if (!rule->name().isEmpty() && candidate->name() != rule->name())
        return false;
    if (!rule->type().isEmpty() && candidate->type() != rule->type())
        return false;
    if (candidate->shape() != rule->shape())
        return false;

    const QPointF expectedPosition = rule->position() + QPointF(offset);
    if (qAbs(candidate->x() - expectedPosition.x()) > 0.001
            || qAbs(candidate->y() - expectedPosition.y()) > 0.001
            || qAbs(candidate->width() - rule->width()) > 0.001
            || qAbs(candidate->height() - rule->height()) > 0.001)
        return false;

    if (candidate->polygon() != rule->polygon())
        return false;

    const Properties properties = rule->properties();
    for (Properties::const_iterator it = properties.constBegin();
         it != properties.constEnd(); ++it) {
        if (it.value() == QLatin1String("*")) {
            if (!candidate->properties().contains(it.key()))
                return false;
        } else if (candidate->property(it.key()) != it.value()) {
            return false;
        }
    }
    return true;
...
static bool objectPatternMatches(const ObjectGroup *workingLayer,
                                 const ObjectGroup *ruleLayer,
                                 const QRegion &ruleRegion,
                                 const QPoint &offset,
                                 bool *defined)
{
    const QList<MapObject*> ruleObjects = objectsInRegion(
                const_cast<ObjectGroup*>(ruleLayer), ruleRegion);
    if (defined)
        *defined = !ruleObjects.isEmpty();
    if (ruleObjects.isEmpty())
        return false;

    foreach (MapObject *ruleObject, ruleObjects) {
        bool found = false;
        foreach (MapObject *candidate, workingLayer->objects()) {
            if (sameRuleObject(candidate, ruleObject, offset)) {
                found = true;
                break;
            }
        }
...
static bool compareObjectGroupTo(const ObjectGroup *setLayer,
                                 ...
                                 const QPoint &offset)
{
    bool anyPositiveDefined = false;
    bool positiveMatched = false;
    foreach (ObjectGroup *ruleLayer, listYes) {
        bool defined = false;
        if (objectPatternMatches(setLayer, ruleLayer, ruleRegion, offset, &defined))
            positiveMatched = true;
        anyPositiveDefined |= defined;
    }
    if (anyPositiveDefined && !positiveMatched)
        return false;

    foreach (ObjectGroup *ruleLayer, listNo) {
        bool defined = false;
        if (objectPatternMatches(setLayer, ruleLayer, ruleRegion, offset, &defined)
                && defined)
            return false;
    }
    return anyPositiveDefined || !listNo.isEmpty();
}
                               ...
                               const RuleOutput *layerTranslation)
{
    for (int i = 0; i < layerTranslation->keys().size(); ++i) {
        ...
        for (const QRect &rect : region) {
            ...
            if (fromTileLayer) {
                TileLayer *toTileLayer = to ? to->asTileLayer() : nullptr;
                if (!toTileLayer) {
                    qWarning() << "Automapping skipped output layer"
                               << from->name()
                               << "because destination index"
                               << layerTranslation->value(from)
                               << "is not a tile layer.";
                    continue;
                }
                ...
                ObjectGroup *toObjectGroup = to ? to->asObjectGroup() : nullptr;
                if (!toObjectGroup) {
                    qWarning() << "Automapping skipped output layer"
                               << from->name()
                               << "because destination index"
                               << layerTranslation->value(from)
                               << "is not an object layer.";
                    continue;
                }
                ...
                qWarning() << "Automapping skipped unsupported output layer"
                           << from->name() << "of type" << from->type();
            }
        }
    }
}

...
QVariant TilesetModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::ToolTipRole) {
        if (Tile *tile = tileAt(index)) {
            QString tilesetName = QFileInfo(mTileset->imageSource()).completeBaseName();
            if (tilesetName.isEmpty())
                tilesetName = mTileset->name();
            const QString tileName = QStringLiteral("%1_%2").arg(tilesetName).arg(tile->id());
            const QString layerName = TilesetManager::instance()->layerName(tile);
            return layerName.isEmpty()
                    ? tileName
                    : tr("%1\nLayer: %2").arg(tileName, layerName);
        }
    }
#endif

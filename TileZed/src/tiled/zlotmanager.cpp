...
void ZLotManager::setMapDocument(MapDocument *mapDoc)
{
    if (mapDoc != mMapDocument) {
        ...
        if (mMapDocument) {
            clearDocumentState();
            mapDocument()->disconnect(this);
            WorldEd::WorldEdMgr::instance()->disconnect(this);
        }
...
void ZLotManager::clearDocumentState()
{
    if (mMapDocument) {
        const QList<MapObject *> mapObjects = mMapObjectToLot.keys();
        for (MapObject *mapObject : mapObjects) {
            MapComposite *lot = mMapObjectToLot.value(mapObject);
            mMapDocument->mapComposite()->removeMap(lot);
            emit lotRemoved(lot, mapObject);
        }

        const QList<WorldCellLot *> worldLots = mWorldCellLotToMC.keys();
        for (WorldCellLot *worldLot : worldLots) {
            MapComposite *lot = mWorldCellLotToMC.value(worldLot);
            mMapDocument->mapComposite()->removeMap(lot);
            emit lotRemoved(lot, worldLot);
        }
    }

    mMapObjectToLot.clear();
    mMapObjectToInfo.clear();
    mMapsLoading.clear();
    mWorldCellLotToMC.clear();
    mWorldCellLotToMI.clear();
    mMapsLoading2.clear();
